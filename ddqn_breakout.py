...
class DDQN_Agent:
    ...
    def replay(self, batch_size):
        ...
        action = np.vstack(np.array(minibatch)[:,1]).astype(int)
        reward = np.vstack(np.array(minibatch)[:,2]).astype(float)
        next_state = np.vstack(np.array(minibatch)[:,3]).astype(float)
        done = np.vstack(np.array(minibatch)[:,4]).astype(int)+0
        ...
        target[:,action[:,0]] = reward + (1-done)*self.gamma*target_q_values[:,best_actions]
